...
async function createAgentSession(disposables: DisposableStore, options?: {
	...
	const stateManager = disposables.add(new class extends AgentHostStateManager {
		override getSessionState() {
			if (!options?.sessionCustomizations) {
				return undefined;
			}
